        ...
        public ActionResult Edit(Int32? Locationid)
        {
            using (var context = new ChossonKallahAdmin.EF6.ChossonKallah())
            {
                try
                {
                    ...
                    sl.Text = "--Please Select--";
                    sl.Value = "0";
                    sl.Selected = (string.IsNullOrEmpty(ResultLocation.LocationName) ? true : false);
                    lstObj.Add(sl);
                    foreach (DataRow dr in dt.Rows)
                    {
                        sl = new SelectListItem();
                        sl.Text = Convert.ToString(dr["FullName"]);
                        sl.Value = Convert.ToString(dr["FullName"]);
                        sl.Selected = (string.IsNullOrEmpty(ResultLocation.LocationName) ? false : ((ResultLocation.LocationName == Convert.ToString(dr["FullName"])) ? true : false));
                        lstObj.Add(sl);
                    }
                    ViewBag.StateData = lstObj;
        ...
        public ActionResult Edit(Location Obj_Location)
        {
            try
            {
                using (var context = new ChossonKallahAdmin.EF6.ChossonKallah())
                {
                    ...
                    var result = context.Database.SqlQuery<Location>("sp_Location_CheckLocationExists_Update  @Locationid,@Locationname", LocationidPara, LocationnamePara).ToList();
                    if (result.Count > 0)
                    {
                        ViewBag.LocationExists = "Location name is already exists.";
                        DataTable dt = new DataTable();
