        ...
        public void GetPosition()
        {
            ...
            for (int i = 1; i < 5; i++)
            {
                if (i == 1)
                {
                    ...
                    slPosition.Text = SessionUtilities.Position.Top.ToString();
                    slPosition.Value = SessionUtilities.Position.Top.ToString();
                    PositionName.Add(slPosition);
                }
                if (i == 2)
                {
                    slPosition = new SelectListItem();
                    slPosition.Text = SessionUtilities.Position.Right.ToString();
                    slPosition.Value = SessionUtilities.Position.Right.ToString();
                    PositionName.Add(slPosition);
                }
                if (i == 3)
                {
                    slPosition = new SelectListItem();
                    slPosition.Text = SessionUtilities.Position.Bottom.ToString();
                    slPosition.Value = SessionUtilities.Position.Bottom.ToString();
                    PositionName.Add(slPosition);
                }
                if (i == 4)
                {
                    slPosition = new SelectListItem();
                    slPosition.Text = SessionUtilities.Position.Left.ToString();
                    slPosition.Value = SessionUtilities.Position.Left.ToString();
                    PositionName.Add(slPosition);
                }
            }
        ...
        public ActionResult Delete(Int32 AdsBannerId)
        {
            try
            {
                using (var context = new ChossonKallahAdmin.EF6.ChossonKallah())
                {
                    ...
                    context.SaveChanges();
                    var adsbannerSequenceidPara = new SqlParameter("@adsbannerid", obj_AdsBanner.AdsBannerID);
                    context.Database.ExecuteSqlCommand("sp_AdsBanner_delete @adsbannerid", adsbannerSequenceidPara);
