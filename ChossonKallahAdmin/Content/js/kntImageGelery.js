function imagegeleryKO()
{
    ...
    self.Addnew = function () {
        ...
        document.getElementById("add").innerText = "Add Image gallery";
        if(document.getElementById("area").val != "");
        {
    ...
    self.checkvalidation = function () {
        ...
        if (self.imagetitle() == "") 
        {
            self.MessageError(self.MessageError() + "<br/> - Image Title is required.");
            IsError = false;
        }
    ...
    self.getselectedpages = function (images) {
        self.Success(false);
        self.Error(false);
        self.id(images.id);
        self.imagename(images.imagename);
        self.imagetitle(images.imagetitle);
        self.isactive(images.isactive);
        self.index(false);
        $("#area").text(images.imagename);
        document.getElementById("viewimg").hidden = false;
        document.getElementById("add").innerText = "Edit Image Gallary";
        $("a#viewimg").attr('href', "../UploadImages/" + images.imagename);
        self.insert(true);
        self.updatebtn(true);
        self.savebtn(false);
        $('#txtnewsdat').focus();
    ...
    self.currentPage = ko.computed(function () {
        ...
        return self.images.slice(startIndex, endIndex);
    });
    ...
    function getdata() {
        ...
        $.ajax({
            type: "GET",
            url: '/ImageGelery/search?PageSize=' + pagesize + "&PageNo=" + self.currentPageIndex() + "&imagetitle=" + Filname,
            Contenttype: "application/JSON",
            success: function (data) {
                self.images(data);
                $('#loader').hide();
    ...
    self.SortData = function () {
        ...
        $.ajax({
            type: "GET",
            url: "/ImageGelery/search?PageSize=" + pagesize + "&PageNo=" + pageno + "&imagetitle=" + Filname + "&sort=" + sort,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (data) {
                self.images(data); gettotalrecord();//GetEmployees();
                $('#loader').hide();
            },
            error: function (error) {
    ...
    self.Search = function (images) {
        ...
        $.ajax({
            type: "GET",
            url: "/ImageGelery/Search?PageSize=" + pagesize + "&PageNo=" + pageno + "&imagetitle=" + Filname,
            contentType: "application/json; charset=utf-8",
            async: false,
            dataType: "json",
            success: function (data) {
    ...
    function gettotalrecord() {
        ...
        $.ajax({
            type: "GET",
            url: '/ImageGelery/search?PageSize=' + pagesize + "&PageNo=-1" + "&imagetitle=" + Filname,
            Contenttype: "application/JSON",
            success: function (data) {
    ...
    self.changestatus = function (images) {

        var isactive = images.isactive;
        var id = images.id;

        $.ajax({
            type: "POST",
            url: "/ImageGelery/UpdateActive?id=" + id + "&Active=" + isactive,
            contentType: "application/json",
            success: function (data) {
    ...
    self.Update = function (data) {
        data.imagename($('#area').val());
        if (self.checkvalidation()) {
            self.Error(false);
            $.ajax(
                {
                    type: "POST",
                    url: "/ImageGelery/Update",
                    data: ko.toJSON(data),
                    contentType: "application/json",
                    async: true,
                    success: function (data) {
    ...
    self.deletepopup = function (images) {
        $("#myModal").modal('show');

        $('#DeleteID').val(images.id);
